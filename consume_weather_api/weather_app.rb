...
class WeatherApp
  ...
  def initialize(client:, api_key:)
    # Convert the passed client name string to its class constant and initialize it with the api key
    @client = Module.const_get(client)::Client.new(api_key)
  end
  ...
  def resolve_required_data_keys(data:, required_data_keys:)
    ...
    required_data_keys.each do |key|
      # For every required data key we check if there is a private
      # method defined with the same name as the key and call it
      resolved_data_hash[key] = send(key, data) if self.class.private_method_defined?(key)
    end
